...
struct ReviewMissionView: View {
    ...
    private func statTile(_ value: String, _ label: LocalizedStringKey, _ icon: String) -> some View {
        VStack(spacing: 4) {
            ...
            Text(label).font(.caption2).foregroundStyle(Brand.text.opacity(0.7))
        }
        ...
        return HStack(spacing: 6) {
            ForEach(0..<count, id: \.self) { i in
                ...
                Text(char)
                    ...
                    .background(Brand.text.opacity(0.08), in: RoundedRectangle(cornerRadius: 10))
    ...
    private var slotColor: Color {
        guard revealed, let out = session.lastOutcome else { return Brand.text.opacity(0.15) }
