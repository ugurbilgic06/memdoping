...
struct ChunkingMissionView: View {
    ...
    private var splittableNumber: some View {
                ...
                if index > 0 {
                    Button {
                        ...
                        RoundedRectangle(cornerRadius: 3)
                            .fill(session.breaks.contains(index) ? Brand.mark : Brand.text.opacity(0.15))
        ...
        .background(Brand.text.opacity(0.06), in: RoundedRectangle(cornerRadius: 18))
    ...
    private var entryDisplay: some View {
        HStack(spacing: 10) {
            ForEach(Array(session.chunks.enumerated()), id: \.offset) { chunkIndex, chunk in
                ...
                HStack(spacing: 2) {
                    ForEach(Array(chunk.enumerated()), id: \.offset) { digitIndex, _ in
                        ...
                        Text(typed.map(String.init) ?? "•")
                            ...
                            .foregroundStyle(typed == nil ? Brand.text.opacity(0.25) : Brand.text)
                            .frame(width: 26)
                ...
                .background((chunkFilled ? Brand.accent.opacity(0.22) : Brand.text.opacity(0.07)),
        ...
        HStack(spacing: 10) {
            ForEach(Array(groups.enumerated()), id: \.offset) { i, group in
                Text(group.map(String.init).joined())
                    ...
                    .background(
                        (highlight ? Brand.accent.opacity(0.3) : Brand.text.opacity(0.08)),
